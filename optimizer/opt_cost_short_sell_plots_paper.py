"""
Grid plots for the paper for the Short Sell Constraint
...
c_list = [None, -0.6, -0.2, 0]  # Maximum short sell constraint
...
if __name__ == "__main__":
    ...
    # Set up an array to keep answers for different value of c
    ...
    for i, kappa in enumerate(kappa_list):  # ([kappa_list[0]]):
        for j, lambd in enumerate(lambd_list):  # ([lambd_list[0]]):
            ...
            for u, c in enumerate(c_list):
                ...
                def L_func(t):
                    return c if c is not None else -np.finfo(np.float64).max
