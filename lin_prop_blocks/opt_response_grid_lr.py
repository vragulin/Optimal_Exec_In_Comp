"""  Solve for best response within the OW-with-Blocks model
    Run the model for a range of parameters and plot the results as a grid
...
FUNC_PARAMS = {"sigma": 3}  # {}
OTHER_STRAT_CODE = "a Risk-Averse"  # "a Risk-Neutral" "an Eager"
...
def plot_curves(c: CostModel, ax: Any, **kwargs) -> None:
    ...
    ax.set_title(f'λ={c.strats[B].lambd}, ρ={c.rho}', fontsize=12)
    ax.set_xlabel('Time')
...
def plot_curves_grid(rho_list: list[float], lambd_list: list[float], func: callable,
                     N: int = 20, **kwargs) -> None:
    """ Plot curves for a range of parameters """

    trader = kwargs.get('trader', A)
    params = kwargs.get('func_params', {})

    fig, axs = plt.subplots(len(rho_list), len(lambd_list), figsize=(15, 15))
    title_str = (f'Best Response to {OTHER_STRAT_CODE} Passive Adversary, Linear Propagator Model\n'
                 f'trader={trader_code(trader)}, N={N}')
    if "sigma" in params:
...
def main():
    plot_curves_grid(rho_list=RHO_LIST, lambd_list=LAMBDA_LIST,
                     func=FUNC, func_params=FUNC_PARAMS, N=N)
