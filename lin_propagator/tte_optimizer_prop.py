...
WIGGLE_FILTER = 0
WIGGLE_INCREASE = 0.01
MAX_OTHER_COST_INC = 0.05  # Increase in the other cost function in relative terms
...
class State:
    ...
    def reg_term(self, x, x_prev):
        ...
        # y2 = np.linalg.norm(x) * REG_FACTOR_WIGGLE
        f = np.exp(y1) - (1 + y1) + y2
        # if np.abs(f) > 1:
        #     print(f"y = {y:.4f}, reg term: {f:.4f}")
        return f
